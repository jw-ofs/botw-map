...
def main():
    ...
    if os.path.exists(armor_path):
        ...
        # one marker per buyable/earnable SET at its source; ring-jitter sets that share a place
        CURATED = {"Champion's Tunic", "Thunder Helm", "Sand Boots", "Snow Boots", "Warm Doublet"}
        qualifying = [s for s in asets if s.get("mapMarker") and s.get("place")
                      and (len(s["pieces"]) >= 2 or s["set"] in CURATED)]
        byplace = {}
        for s in qualifying:
            c = find_place(s["place"])
            if c:
